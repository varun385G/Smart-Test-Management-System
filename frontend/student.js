  ...
  try {
    ...
    if (!res.ok) {
      if (res.status === 403 && data.scheduledStart) {
        _storeCredentials(testId, name, reg);
      ...
      if (res.status === 403 && data.scheduledEnd) {
        ...
        if (endedSecondsAgo <= 90) {
          ...
          await _issueToken(testId, reg, name);
    ...
    if (data.scheduledStart) {
      ...
      if (minsLeft > 0) {
        localStorage.setItem('examScheduledStart', data.scheduledStart);
        showNotStartedCountdown(data.scheduledStart);
        return;
      }
    }
...
function _storeCredentials(testId, name, reg) {
...
async function _issueToken(testId, reg, name) {
  try {
...
async function _reissueTokenAndRedirect(delayMs) {
  const tid  = localStorage.getItem('testId');
  const reg  = localStorage.getItem('studentReg');
  const name = localStorage.getItem('studentName') || '';
  if (tid && reg) {
  ...
  setTimeout(() => { location.href = '/exam.html'; }, delayMs || 1000);
}