...
app.post('/api/tests/create', async (req, res) => {
  try {
    const { title, password, duration, questions, security, shuffleQuestions, shuffleOptions, staffId } = req.body;
    const test = new Test({
...
app.put('/api/tests/:testId/update', async (req, res) => {
  try {
    ...
    const { title, password, duration, questions, security, shuffleQuestions, shuffleOptions } = req.body;
...
app.post('/api/student/validate', async (req, res) => {
  ...
  if (attempted) return res.json({ attempted: true, resultsPublished: test.resultsPublished });
...
app.post('/api/exam/submit', async (req, res) => {
  try {
    ...
    test.questions.forEach((q, i) => {
      ...
      if (q.type === 'MCQ' && ans === q.correctIndex) score += marks;
      if (q.type === 'MSQ' && Array.isArray(ans) && Array.isArray(q.correctIndexes) &&
          [...ans].sort().join(',') === [...q.correctIndexes].sort().join(',')) score += marks;
      if (q.type === 'NAT' && Number(ans) === Number(q.correctValue)) score += marks;
    });

    const total = test.questions.reduce((s, q) => s + (Number(q.marks) || 1), 0);
